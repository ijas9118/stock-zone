...
`;
  ...
  // Map mixBlendMode to Tailwind class
  const blendClasses: Record<string, string> = {
  ...
  const blendClass = blendClasses[mixBlendMode] || "";

  return (
    <div
      ref={containerRef}
      className={`relative h-full w-full overflow-hidden ${blendClass} ${className || ""}`.trim()}
    />
  );
};
